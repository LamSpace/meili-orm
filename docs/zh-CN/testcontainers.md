[English](../testcontainers.md)

# Testcontainers 集成（meili-orm-testcontainers）

## 结论先行

`meili-orm-testcontainers` 是 opt-in 坐标（**不在 starter 聚合内**，见
[README](../../README.zh-CN.md)），测试作用域按需引入，提供两件事：

1. **`MeiliSearchContainer`**：类型化 Meilisearch 服务容器，可脱离 Spring 独立使用；
2. **`@ServiceConnection` 服务连接桥接**：容器字段加一行注解，桥接自动产出
   `MeiliConnectionDetails` bean，装配链（`Config → Client → Operations`）零手写
   配置指向容器。
...
上下文不写任何 `meili.url` / `meili.api-key`：连接信息的唯一来源是被桥接的容器。
建议测试属性配 `meili.wait-task=true`（写路径同步等待任务终态，断言无竞态）；
索引由默认的 `auto-init=create-if-missing` 在启动期建立。

### 优先级（高 → 低）

| 来源 | 行为 |
|---|---|
| 用户自有 `MeiliConnectionDetails` bean | 桥接退避（连同属性默认一起退避），`Config` 用用户 bean |
| `@ServiceConnection` 桥接 bean | 属性默认退避，`Config` 用容器 URL/密钥 |
| `meili.url` / `meili.api-key` 属性 | 以上两者都不存在时的默认实现 |

桥接对用户 bean 的退避由本模块内的自动配置守卫完成：Boot 的服务连接注册发生在
测试上下文定制阶段，早于用户 `@Bean` 方法解析，无法自发看见用户 bean；守卫在
全部 bean 定义解析完成后、实例化之前，移除"带 `@ServiceConnection` 标记且上下文
同时存在用户自有 details bean"场景下的桥接 bean 定义。
...
`MeiliConnectionDetails` bean——这正是既有退避契约的声明路径：
...
| 3.5.x | ✓ 支持（编译基线；本模块真机 IT 实证） | ✓ |
| 3.1–3.4 | 不承诺（超出本工程 Boot 支持面，[README](../../README.zh-CN.md) 所声明）。注：服务连接发现 key 自 3.1 GA 起即与新代同名（3.1.12–3.4.13 构件实测），机制层面可达，但容器基类跨代二进制兼容未逐代验证 | ✓ |
| ≤3.0 / 2.x | ✗ 无服务连接机制 | ✓ |
