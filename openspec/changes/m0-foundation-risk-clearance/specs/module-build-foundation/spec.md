...
系统 SHALL 提供聚合根 pom（packaging=pom），含 `meili-orm-core`、`meili-orm-spring-boot-autoconfigure`、`spring-boot-starter-meili-orm` 三个模块；`meili-orm-core` 的编译期依赖 SHALL NOT 含任何 `org.springframework` 坐标。

#### Scenario: 全量构建通过

- **WHEN** 执行 `mvn -s /home/lam/repo/settings.xml clean verify`
- **THEN** 三个模块均构建成功（BUILD SUCCESS），无测试编译错误
