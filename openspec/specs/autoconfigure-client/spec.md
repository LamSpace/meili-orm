...
系统 SHALL 在 `meili.` 前缀下提供配置属性：`enabled`（默认 `true`）、`url`（默认 `http://localhost:7700`）、`api-key`（默认空）、`wait-task`（默认 `false`）、`wait-timeout`（默认 `5s`）、`index.auto-init`（默认 `create-if-missing`）、`index.on-settings-drift`（默认 `warn`）。属性 SHALL NOT 包含连接/读超时项（SDK 限制）。
