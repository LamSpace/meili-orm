...
4. **源码语言为英文**。全部 Java 源文件（含 test 与 examples）的注释/Javadoc（含行尾注释）、根与各模块 pom 的 XML 注释与 `<description>`、`src/main` 的异常消息与日志文案一律英文；承担测试/演示数据语义的中文字面量（CJK 无损验证样本、演示书目数据等）按变更留档的白名单保留。每个 Java 文件须携带 `LICENSE` 的 Apache-2.0 短头部（`etc/license-header.txt`），缺失或篡改由 `license-maven-plugin` 门禁使构建失败。
